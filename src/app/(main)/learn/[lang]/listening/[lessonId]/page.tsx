...
export default function ListeningDetail({ params }: { params: { lang: string; lessonId: string } }) {
  const { data } = useLessons(params.lang, "listening");
  ...
  if (!lesson) {
    return (
      <div className="mx-auto max-w-3xl p-6 space-y-3">
        <Link className="underline" href={`/learn/${params.lang}/listening`}>← Back</Link>
